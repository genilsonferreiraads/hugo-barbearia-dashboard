...
export const ServiceModal: React.FC<ServiceModalProps> = ({ isOpen, onClose, service }) => {
  ...
  useEffect(() => {
    if (service) {
      ...
      setPrice(service.price.toFixed(2).replace('.', ','));
    } else {
      setName('');
      setPrice('');
    }
          ...
          <div className="relative flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="p-2.5 bg-gradient-to-br from-primary to-red-600 rounded-xl shadow-lg shadow-primary/25">
                <Icon name={service ? "edit" : "add"} className="text-white text-2xl" />
              </div>
              <div>
          ...
          <div>
            <label className="block">
              <div className="flex items-center gap-2 mb-1.5">
                <Icon name="content_cut" className="text-primary text-base" />
